...
def classify_by_province(data):
    result = dict()
    # exception_address = []
    for address in data:
        # find the key, which is either xx省 or xx市
        # exception would be: 新疆,宁夏,内蒙古
        if address[1][:2] == "新疆":
            add_key = "新疆"
        elif address[1][:2] == "宁夏":
            add_key = "宁夏"
        elif address[1][:3] == "内蒙古":
            add_key = "内蒙古"
        else:
            end_index = address[1].find("省")
            if end_index == -1:
                end_index = address[1].find("市")
                # if end_index == -1 or end_index >= 3:
                #     exception_address.append(address)
                #     continue

            # the key that is adding to the result dictionary
            add_key = address[1][:end_index+1]

        # if the key already in the dict
        if add_key in result:
            # append the address info
            result[add_key].append(address)
        # the key not yet in the dict
        else:
            # add new pair of key and value into the dict
            result[add_key] = [address]
    return result


if __name__ == "__main__":
    # data = [['王*龙', '北京市海淀区苏州街大恒科技大厦南座4层'], ['庞*飞', '新疆昌平区汇德商厦四楼403']]
    result = classify_by_province(data)
    print(result)
